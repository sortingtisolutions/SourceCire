		...
		if($fileName && in_array($fileType, $file_mimes)){
			if(is_uploaded_file($_FILES['file']['tmp_name'])){
				...
				while(($LoadProducts = fgetcsv($csv_file)) !== FALSE){
					...
					$qry2 = "SELECT st.str_id FROM ctt_stores AS st WHERE st.str_name = '$LoadProducts[10]' UNION SELECT 0 LIMIT 1";
					$rest = $this->db->query($qry2);
					$rst = $rest->fetch_object();
					$store = $rst->str_id;
					
					// Busca el id del proveedor
					$qry2 = "SELECT sp.sup_id FROM ctt_suppliers AS sp WHERE sp.sup_business_name = '$LoadProducts[11]' UNION SELECT 0 LIMIT 1";
					$rest = $this->db->query($qry2);
					$rst = $rest->fetch_object();
					$supplier = $rst->sup_id;
					...
					if (is_numeric($LoadProducts[2])) {
						# Costo
						$costo = $LoadProducts[2];
					}else{
						$costo = '';
						$estatus = $estatus . "Costo, ";
					}
					// Verificar que el tipo de moneda exista
					if (strlen($LoadProducts[9]) == 3) {
						// Moneda
						$qry2 = "SELECT cn.cin_id FROM ctt_coins AS cn WHERE cn.cin_code = '$LoadProducts[9]' UNION SELECT 0 LIMIT 1";
						$rest = $this->db->query($qry2);
						$rst = $rest->fetch_object();
						$coin = $rst->cin_id;
					...
					//if ( && (is_numeric($LoadProducts[1]) || $LoadProducts[1]== '') && is_numeric($LoadProducts[2])) {
						...
						$qry = "INSERT INTO ctt_load_series(ser_sku, ser_serial_number, ser_cost, ser_brand, ser_cost_import, ser_import_petition, ser_sum_ctot_cimp,ser_no_econo,ser_comments,cin_id,str_id,sup_id, ser_situation, ser_stage, result, prd_id, ser_status)
							VALUES ('$LoadProducts[0]', '$LoadProducts[1]','$costo', '$LoadProducts[3]', '$LoadProducts[4]', '$LoadProducts[5]', '$LoadProducts[6]', '$LoadProducts[7]', '$LoadProducts[8]', '$coin', '$strId', '$supId', 'D', 'D', '$estatus', '$prdId', 1)";
						$this->db->query($qry);
