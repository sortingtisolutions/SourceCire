...
if ($empid == '1' || $empid == '2'){
    ...
    $qry = "SELECT prd.prd_name, prd.prd_level, pjtcn_prod_name, pdt.pjtdt_prod_sku, 
        ...
        INNER JOIN ctt_projects_type AS pt ON pt.pjttp_id = pj.pjttp_id
        INNER JOIN ctt_location AS loc ON loc.loc_id = pj.loc_id
        WHERE pcn.pjt_id=$prdId AND em.emp_id=$empid
        ORDER BY pdt.pjtdt_id;"; // 11-10-23
}
$res = $conn->query($qry);
...
$html = '
    <section>
        <div class="container">
        <div style="height:80px;"></div><table class="table-data bline-d tline">
        <tr>
            <td class="rline half">
        ...
        <table class="table-data bline tline" style="text-align: center">
            <tr>
                <td>
                    ...
                    Impresion con accesorios de entrada de un proyecto
                    ...
                    <table autosize="1" style="page-break-inside:void" class="table-data bline-d">
                        ...
                        for ($i = 0; $i<count($items); $i++){
                            ...
                            if ($section == '1') {
                                ...
                                $typeprd       = $items[$i]['prd_type_asigned'] ; //  --------
                                $comment       = $items[$i]['ser_comments'];
                                if ($typeprd == 'PV' || $typeprd == 'PF' || $typeprd == 'PI') {
