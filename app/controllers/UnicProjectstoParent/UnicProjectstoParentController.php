<?php
    ...
    require_once ROOT . FOLDER_PATH . '/app/models/UnicProjectstoParent/UnicProjectstoParentModel.php';
    require_once LIBS_ROUTE .'Session.php';

class UnicProjectstoParentController extends Controller
{
    private $session;
    public $model;
    ...
    {
        $this->model = new UnicProjectstoParentModel();
...
// Lista los productos relacionados al paquete
    ...
    {
        $params =  $this->session->get('user');
        $result = $this->model->listProductsPack($request_params);
        $i = 0;
        while($row = $result->fetch_assoc()){
            $rowdata[$i] = $row;
            $i++;
